...
class Auth {
  ...
  Future<void> updateProfile({
    ...
    try {
      loadingDialogBox(context, 'Updating Profile');

      Map<String, dynamic> dataToUpdate = {};

      if (name != null) dataToUpdate['name'] = name;
      if (email != null) dataToUpdate['email'] = email;
      if (mobile != null) dataToUpdate['mobile'] = mobile;
      if (address != null) dataToUpdate['address'] = address;

      await users.doc(uid).update(dataToUpdate);

      Navigator.pop(context);
      customSnackBar(context: context, content: 'Profile updated successfully');
    } catch (e) {
      Navigator.pop(context);
      customSnackBar(
          context: context, content: 'Failed to update profile: $e');
    }
  }
}
